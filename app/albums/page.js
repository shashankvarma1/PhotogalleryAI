...
export default function Albums() {
  ...
  // View mode: 'albums' | 'years'
  const [viewMode, setViewMode] = useState('albums');

  // Year view state
  const [yearGroups, setYearGroups]       = useState([]); // [{ year, photos, cover_url }]
  const [yearLoading, setYearLoading]     = useState(false);
  const [expandedYear, setExpandedYear]   = useState(null); // year number or null
  ...
  // Fetch year groups when switching to year view
  useEffect(() => {
    if (viewMode === 'years' && yearGroups.length === 0) {
      fetchYearGroups();
    }
  }, [viewMode]);
  ...
  // ── Build year groups from all photos ────────────────────────────────────
  const fetchYearGroups = async () => {
    ...
    try {
      ...
      // Group by year using date_taken, fallback to uploaded_at
      const byYear = {};
      ...
      // Sort photos within each year newest-first, pick cover
  ...
  const YearCard = ({ group }) => (
    ...
    >
      {/* Mosaic cover — same height as AlbumCard */}
      ...
      {/* Label row — matches AlbumCard info section exactly */}
      <div style={{ padding: '14px 16px' }}>
        <div style={{
          fontFamily: "'Syne',sans-serif", fontSize: 14, fontWeight: 700, color: '#111',
        ...
        <span style={{
          ...
          color: 'rgba(17,17,17,0.4)', letterSpacing: '0.06em',
        }}>
      ...
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .view-toggle {
          ...
          border: 1.5px solid rgba(17,17,17,0.12); background: rgba(17,17,17,0.04);
        }
        .view-toggle button {
      ...
      <main style={{ marginLeft: '240px', marginTop: '62px', padding: '36px 32px', minHeight: 'calc(100vh - 62px)', background: '#f2efe9' }}>
        ...
        <div className="fu-1" style={{ marginBottom: 28 }}>
          <div className="view-toggle">
            <button
              className={viewMode === 'albums' ? 'active' : 'inactive'}
              onClick={() => setViewMode('albums')}
            >
              📁 Albums
            </button>
            <button
              className={viewMode === 'years' ? 'active' : 'inactive'}
