...
export default function Gallery() {
  ...
  return (
    <>
      ...
      <main style={{
        marginLeft: '0', marginTop: '64px', padding: '1.5rem', paddingBottom: '90px',
        minHeight: 'calc(100vh - 64px - 90px)', backgroundColor: '#f8fafc', transition: 'margin-left 0.3s ease',
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem', flexWrap: 'wrap', gap: '1rem' }}>
          ...
          <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center', flexWrap: 'wrap' }}>
            {photos.length > 0 && (
              <button onClick={toggleSelectMode} style={{ padding: '0.8rem 1.25rem', backgroundColor: selectMode ? '#f3f4f6' : 'white', color: '#374151', border: '1px solid #d1d5db', borderRadius: '8px', fontWeight: '600', cursor: 'pointer' }}>
                {selectMode ? 'Cancel' : 'Select'}
              </button>
            )}
            {selectMode && (
              <>
                <button onClick={selectAll} style={{ padding: '0.8rem 1.25rem', backgroundColor: 'white', color: '#374151', border: '1px solid #d1d5db', borderRadius: '8px', fontWeight: '600', cursor: 'pointer' }}>
                  {selectedIds.size === photos.length ? 'Deselect All' : 'Select All'}
                </button>
                {selectedIds.size > 0 && (
                  <button onClick={() => handleDelete([...selectedIds])} disabled={deleting} style={{ padding: '0.8rem 1.25rem', backgroundColor: deleting ? '#9ca3af' : '#dc2626', color: 'white', border: 'none', borderRadius: '8px', fontWeight: '600', cursor: deleting ? 'not-allowed' : 'pointer' }}>
            ...
            {!selectMode && (
              <label style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', padding: '0.8rem 1.5rem', backgroundColor: uploading ? '#9ca3af' : '#2563eb', color: 'white', borderRadius: '8px', fontWeight: '600', cursor: uploading ? 'not-allowed' : 'pointer', transition: 'background-color 0.2s' }}
                onMouseOver={(e) => { if (!uploading) e.currentTarget.style.backgroundColor = '#1d4ed8'; }}
                onMouseOut={(e) => { if (!uploading) e.currentTarget.style.backgroundColor = uploading ? '#9ca3af' : '#2563eb'; }}
              >
        ...
        {photos.length > 0 ? (
          <div style={{ marginBottom: '3rem' }}>
            <h2 style={{ fontSize: '1.4rem', fontWeight: '600', color: '#4b5563', marginBottom: '1.25rem' }}>
              ...
              {selectMode && selectedIds.size > 0 && <span style={{ fontSize: '1rem', color: '#2563eb', marginLeft: '0.75rem' }}>{selectedIds.size} selected</span>}
            </h2>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: '1rem' }}>
              {photos.map((photo) => (
                <div key={photo.id}
                  onClick={() => { if (selectMode) toggleSelect(photo.id); else { setSelectedPhoto(photo); setShareMsg(''); setShareUsername(''); } }}
                ...
                >
                  ...
                  {selectMode && (
                    <div style={{ position: 'absolute', top: '0.5rem', left: '0.5rem', width: '22px', height: '22px', borderRadius: '50%', backgroundColor: selectedIds.has(photo.id) ? '#2563eb' : 'rgba(255,255,255,0.8)', border: '2px solid white', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 1px 3px rgba(0,0,0,0.3)' }}>
                      {selectedIds.has(photo.id) && <span style={{ color: 'white', fontSize: '13px', fontWeight: 'bold' }}>✓</span>}
                    </div>
                  )}
                </div>
              ))}
          ...
          <div onClick={(e) => e.stopPropagation()} style={{ position: 'relative', maxWidth: '95vw', maxHeight: '90vh', backgroundColor: 'white', borderRadius: '12px', overflow: 'hidden', boxShadow: '0 25px 50px rgba(0,0,0,0.4)', display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ padding: '1rem 1.25rem', borderTop: '1px solid #e5e7eb', backgroundColor: 'white' }}>
              <div style={{ marginBottom: '0.75rem' }}>
                <button onClick={() => handleDelete([selectedPhoto.id])} disabled={deleting} style={{ padding: '0.5rem 1rem', backgroundColor: '#fee2e2', color: '#dc2626', border: 'none', borderRadius: '8px', fontWeight: '600', cursor: 'pointer', fontSize: '0.9rem' }}>
                  🗑 Delete
                </button>
              </div>
              ...
              <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                ...
                />
                <button onClick={handleShare} disabled={sharing || !shareUsername.trim()} style={{ padding: '0.6rem 1.1rem', backgroundColor: sharing ? '#9ca3af' : '#2563eb', color: 'white', border: 'none', borderRadius: '8px', fontWeight: '600', cursor: sharing ? 'not-allowed' : 'pointer', fontSize: '0.9rem', whiteSpace: 'nowrap' }}>
                  {sharing ? '...' : '🔗 Share'}
                </button>
              </div>
