...
// ── Step tracker — shows what the agent did 
...
export default function AgentPage() {
  ...
  const [history,     setHistory]     = useState([]);  // Full OpenAI conversation history
  const [input,       setInput]       = useState("");
  const [loading,     setLoading]     = useState(false);
  const [pendingSteps,setPendingSteps]= useState(null);
  const [confirmation,setConfirmation]= useState(null); // Waiting for user confirm
  const [pendingHistory, setPendingHistory] = useState(null); // History snapshot at confirmation point
  ...
  const sendMessage = async (text, resumeHistory = null) => {
    ...
    try {
      ...
      // Update the full conversation history for next turn
      ...
      setMessages(prev => [
        ...prev,
        { role: "assistant", content: data.message || "Done.", steps: data.steps },
      ]);
    } catch {
      setMessages(prev => [
        ...prev,
  ...
  const handleConfirm = () => {
    ...
    // Resume: inject a user "yes, confirmed" message into history and continue
    const resumeHistory = [
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "32px 40px 120px" }}>
          ...
          {messages.length === 0 && (
            <div style={{ maxWidth: 640, margin: "60px auto 0", textAlign: "center" }}>
              <p style={{ fontFamily: "'Syne', sans-serif", fontSize: 11, fontWeight: 600, letterSpacing: "0.18em", textTransform: "uppercase", color: "rgba(17,17,17,0.35)", marginBottom: 12 }}>
                ✦ Gathrd AI
              </p>
              <h1 style={{ fontFamily: "'Instrument Serif', serif", fontSize: "clamp(28px, 3.5vw, 42px)", fontWeight: 400, fontStyle: "italic", color: "#111", lineHeight: 1.15, marginBottom: 12 }}>
                What would you like to do with your photos?
              </h1>
              <p style={{ fontFamily: "'Syne', sans-serif", fontSize: 14, color: "rgba(17,17,17,0.45)", marginBottom: 32, lineHeight: 1.7 }}>
                Ask anything. I can search, organise, share, and manage your entire library.
              </p>
          ...
          <div style={{ maxWidth: 720, margin: "0 auto" }}>
            {messages.map((msg, i) => {
              ...
              return (
                <div key={i}>
                  {isAssistant && pendingSteps && i === messages.length - 1 && (
