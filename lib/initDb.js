...
export async function initDb() {
  ...
  // Photos — includes AI fields and 384-dim vector
  await pool.query(`
  ...
  // People — named faces the user has tagged
  await pool.query(`
  ...
  // Migrate: add missing columns to existing photos table
  const alterCols = [
  ...
  // Handle vector column: if it exists with wrong dims, we can't easily migrate.
  // Just try to add it — if it exists with 768 dims, the reindex will fail gracefully.
  await pool.query(
  ...
  `).catch(() => {}); // ignore if vector col has wrong dims
