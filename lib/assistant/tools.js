...
export async function executeSearchPhotos(params, username) {
  const {
    query, person_name, emotion, location,
  ...
  try {
    // Always start with uploaded_by = $1
    ...
    // Person filter
    if (person_name) {
      // Step 1: does the person record exist at all?
      ...
      if (!nameCheck.rows.length) {
        // Person was never tagged on the People page
        return {
      ...
      // Step 2: person exists — do they have any linked photos?
      const personId = nameCheck.rows[0].id;
      const linkCheck = await pool.query(
        `SELECT 1 FROM photo_people WHERE person_id = $1
         UNION ALL
         SELECT 1 FROM face_tags   WHERE person_id = $1
         LIMIT 1`,
        [personId]
      );
 
      if (!linkCheck.rows.length) {
        // Tagged but zero photo links — face detection missed them or tagging failed silently
        return {
          photos: [], count: 0, person_tagged_no_photos: true,
          message: `I found "${person_name}" in your tagged people but they aren't linked to any photos yet. This usually means face detection didn't pick them up — try re-scanning on the People page and re-tagging the cluster.`,
        };
      }
 
      // Step 3: all good — build the JOIN
      // Use a fresh $idx for per.username so it doesn't collide with $1 (uploaded_by)
      joinClause = `
        JOIN photo_people pp ON pp.photo_id = p.id
        JOIN people per ON per.id = pp.person_id AND per.username = $${idx++}
      `;
      baseValues.push(username);
      conditions.push(`per.name ILIKE $${idx++}`);
      baseValues.push(`%${person_name}%`);
      hasPeopleJoin = true;
    }
    ...
    // When both person + event query are present: use 0.42 — the person JOIN
    // already narrows the pool, so we can afford a slightly lower bar.
    // When only an event/topic query (no person): use 0.50 to avoid
    // semantically adjacent matches (e.g. "happy" photos for "birthday").
    // When only person_name with no query: threshold is irrelevant (no vector).
    const similarityThreshold = (hasPeopleJoin && query) ? 0.42 : 0.50;
 
    // ── Vector search ──────────────────────────────────────────────────────
    // FIX 1: build a SEPARATE values array so we never mutate baseValues.
    // FIX 2: when person JOIN is active, embed only the event `query` term —
    //   NOT person_name. The JOIN already handles person filtering; mixing the
    //   person's name into the embedding biases similarity toward face/identity
    //   descriptors instead of the event you're actually searching for.
    let photos = [];
    const semanticQuery = query || (!hasPeopleJoin ? person_name : "") || location || "";
    ...
    if (semanticQuery) {
      try {
        ...
        // Fresh copy — don't mutate baseValues
        const vectorValues = [...baseValues, embedding, cap];
        const embIdx = idx;       // $idx = embedding
        const capIdx = idx + 1;   // $idx+1 = cap
        ...
        // Always apply threshold — even with person JOIN, we still want to
        // filter out photos that don't actually match the event query.
        photos = result.rows.filter(r => r.similarity >= similarityThreshold);
      } catch (_vecErr) {
        // vector search failed — fall through to fallback below
      }
    }
 
    // ── Fallback: keyword match on description/place ───────────────────────
    // Only used when vector search fails or returns nothing.
    // For person+event: still restrict to tagged photos via the JOIN,
    // then keyword-match the query term against ai_description.
    if (photos.length === 0) {
      // Fresh copy — baseValues is still clean
      const fallbackValues = [...baseValues, cap];
      const capIdx = idx; // next unused index
...
`;
      ...
      // Always keyword-filter when there's a query — even for person searches.
      // This ensures "srihitha's birthday" in fallback mode doesn't return
      // all of Srihitha's photos, just ones mentioning "birthday".
      photos = result.rows.filter(p => {
    ...
    return {
      ...
      resolved_person: hasPeopleJoin ? person_name : null,
    };
  } catch (err) {
...
export async function executeGetPeopleStats({ person_name } = {}, username) {
  try {
    if (person_name) {
      ...
      if (!personCheck.rows.length) {
        return {
          people: [],
          message: `"${person_name}" isn't tagged yet. Tag their face on the People page!`,
        };
      }
      const personId = personCheck.rows[0].id;
 
      // FIX: add p.uploaded_by = $1 so we only return this user's photos.
      // Previously $1 = username was passed but the WHERE only referenced $2.
  ...
  [username, personId]
    ...
    // ── All people stats (unchanged, was already correct) ──────────────────
