...
// Cache for supabaseAdmin in case static import fails
let _supabase = null;
async function getSupabase() {
  if (supabaseAdmin?.storage) return supabaseAdmin;
  if (_supabase?.storage) return _supabase;
  try { const m = await import("@/lib/supabaseAdmin"); _supabase = m.default || m; return _supabase; } catch {}
  return null;
}
...
export const TOOL_DEFINITIONS = [
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          ...
          person_names:  { type: "array", items: { type: "string" }, description: "Filter to photos containing ALL of these people. Use when user mentions 2+ people together e.g. ['yashu','srihitha']. Each must be a tagged person name." },
          min_faces:     { type: "number",  description: "Minimum face count. Use 2 for group photos." },
          sort:          { type: "string",  enum: ["newest","oldest"], description: "Sort order. Default newest. Use oldest for 'my oldest photos'." },
  ...
  },
  {
    type: "function",
    function: {
      name: "rank_best_photos",
      description: "Rank and return the user's best/top photos scored by Instagram-worthiness: face count, positive emotion, resolution, and recency. Use for 'best photos', 'top 5 photos', 'most Instagram-worthy photos', 'find my best photo from 2024'.",
      parameters: {
        type: "object",
        properties: {
          limit:      { type: "number", description: "Number of top photos to return, default 5." },
          date_year:  { type: "number", description: "Filter to a specific year." },
          date_month: { type: "number", description: "Filter to a specific month." },
          location:   { type: "string", description: "Filter by location." },
          person_name:{ type: "string", description: "Filter to photos of a specific person." },
        },
        required: [],
      },
    },
...
// Refresh URLs using Supabase batch signed URLs for speed
async function refreshPhotoUrls(photos) {
  if (!photos?.length) return photos;
  const withPaths = photos.filter(p => p.storage_path);
  const withoutPaths = photos.filter(p => !p.storage_path);
  if (!withPaths.length) return photos;

  try {
    const sb = await getSupabase();
    if (!sb?.storage) return photos; // no supabase available, return as-is
    // Use createSignedUrls (batch) instead of one-by-one createSignedUrl
    const { data, error } = await sb.storage
      .from("photos")
      .createSignedUrls(withPaths.map(p => p.storage_path), 60 * 60 * 24 * 365);
    ...
    if (error || !data) {
      ...
      return Promise.all(photos.map(async p => {
        ...
        try {
          const { data: d } = await sb.storage.from("photos").createSignedUrl(p.storage_path, 60 * 60 * 24 * 365);
          return d?.signedUrl ? { ...p, url: d.signedUrl } : p;
        } catch { return p; }
      }));
    }

    // Map signed URLs back to photos by storage_path
    const urlMap = {};
    for (const item of data) {
      if (item.signedUrl) urlMap[item.path] = item.signedUrl;
    }
    return [...withPaths.map(p => urlMap[p.storage_path] ? { ...p, url: urlMap[p.storage_path] } : p), ...withoutPaths];
  } catch {
    return photos;
  }
}
...
export async function executeSearchPhotos(params, username) {
  const {
    query, person_name, person_names = [], is_self_query,
    emotion, location,
    date_year, date_month, date_from, date_to,
    min_faces, sort = "newest", limit = 30,
  } = params;
  ...
  try {
    ...
    if (person_name) {
      ...
      // Check linked photos — run in parallel with multi-person lookup below
      const linkCheck = await pool.query(
        `SELECT 1 FROM photo_people WHERE person_id = $1 LIMIT 1`,
        [namedPersonId]
      );
    ...
    // ── Resolve multiple people in parallel ──────────────────────────────────
    let multiPersonIds = [];
    if (person_names && person_names.length > 0) {
      const results = await Promise.all(
        person_names.map(pn => pool.query(
          `SELECT id FROM people WHERE username = $1 AND name ILIKE $2 LIMIT 1`,
          [username, `%${pn}%`]
        ))
      );
      multiPersonIds = results.filter(r => r.rows.length).map(r => r.rows[0].id);
    }
    const hasMultiPeople = multiPersonIds.length >= 2;

    const resolvedPersonId = selfPersonId ?? namedPersonId;
    const hasPeopleFilter = resolvedPersonId !== null || hasMultiPeople;

    // ── Extract keywords ──────────────────────────────────────────────────────
    const keywords = extractKeywords(query || "");

    // ── Sort order (defined early so keyword query can use it) ────────────────
    ...
    if (keywords.length > 0 && hasPeopleFilter) {
      ...
      const kwSql = `
        ...
        WHERE p.uploaded_by = $1
          AND EXISTS (SELECT 1 FROM photo_people pp WHERE pp.photo_id = p.id AND pp.person_id = $2)
          AND (${kwConditions.join(" OR ")})
        ORDER BY sort_date ${sortDir}
        LIMIT ${cap}
      `;
    ...
    if (resolvedPersonId) {
      // Use EXISTS for cleaner query plan - PostgreSQL can short-circuit
      joinClause = `
        JOIN photo_people person_filter
          ON person_filter.photo_id = p.id
         AND person_filter.person_id = $${idx}
      `;
      baseValues.push(resolvedPersonId); idx++;
    }

    // ── Multi-person: require ALL named people to appear in the photo ─────────
    if (hasMultiPeople && !resolvedPersonId) {
      for (const pid of multiPersonIds) {
        conditions.push(`EXISTS (SELECT 1 FROM photo_people pp WHERE pp.photo_id = p.id AND pp.person_id = $${idx})`);
        baseValues.push(pid); idx++;
      }
    }
...
export async function executeRankBestPhotos({ limit = 5, date_year, date_month, location, person_name } = {}, username) {
  try {
    const conditions = ["p.uploaded_by = $1"];
    const values = [username];
    let idx = 2;
    ...
    if (person_name) {
      ...
      if (personRes.rows.length) {
        ...
        joinClause = `
          JOIN photo_people pf ON pf.photo_id = p.id AND pf.person_id = $${idx}
        `;
        values.push(pid); idx++;
      }
    }
    ...
    // Score formula:
    //   emotion bonus: happy/excited = 3pts, surprised/calm = 1pt
    //   face bonus: 1-2 faces = 2pts, 3+ faces = 1pt (solo or big groups slightly less)
    //   resolution bonus: >5MP = 2pts, >2MP = 1pt
    //   recency bonus: within last year = 1pt
    const sql = `
      SELECT p.id, p.url, p.storage_path, p.filename, p.ai_description,
             p.dominant_emotion, p.place_name, p.date_taken, p.uploaded_at,
             p.face_count, p.width, p.height,
             (
               CASE p.dominant_emotion
                 WHEN 'happy'   THEN 3 WHEN 'excited'  THEN 3
                 WHEN 'calm'    THEN 1 WHEN 'surprised' THEN 1
                 ELSE 0 END
               +
               CASE
                 WHEN p.face_count BETWEEN 1 AND 2 THEN 2
                 WHEN p.face_count >= 3 THEN 1
                 ELSE 0 END
               +
               CASE
                 WHEN (p.width * p.height) > 5000000 THEN 2
                 WHEN (p.width * p.height) > 2000000 THEN 1
                 ELSE 0 END
               +
               CASE
                 WHEN COALESCE(p.date_taken, p.uploaded_at) > NOW() - INTERVAL '1 year' THEN 1
                 ELSE 0 END
             ) AS instagram_score
      FROM photos p ${joinClause}
      WHERE ${whereClause}
        AND p.width IS NOT NULL
      ORDER BY instagram_score DESC, COALESCE(p.date_taken, p.uploaded_at) DESC
      LIMIT ${capIdx}
    `;

    const result = await pool.query(sql, values);
    const photos = await refreshPhotoUrls(result.rows);

    return {
      photos,
      count: photos.length,
      ranked: true,
      message: `Top ${photos.length} photos ranked by Instagram-worthiness`,
    };
  } catch (err) {
    console.error("rank_best_photos error:", err);
    return { error: err.message, photos: [] };
  }
}
...
export async function executeTool(toolName, params, username) {
  switch (toolName) {
    ...
    case "rank_best_photos":         return executeRankBestPhotos(params, username);
