 ...
 */
export function buildDescription({ caption = null, filename = "", exif = {}, faceCount = 0, emotion = null, peopleNames = [] ,placeName = null} = {}) {
  const parts = [];
...
} else if (exif?.latitude && exif?.longitude && !isNaN(exif.latitude) && !isNaN(exif.longitude)) {
  parts.push(`GPS coordinates: ${Number(exif.latitude).toFixed(4)}, ${Number(exif.longitude).toFixed(4)}.`);
}
